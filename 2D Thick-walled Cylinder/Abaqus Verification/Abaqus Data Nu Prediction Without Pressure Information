...
idx1 = np.random.choice(np.arange(len(coors)), 1000, replace=False)
...
def pde(x, y):
    ...
    return mx, my, stress_x, stress_y, stress_xy
...
def output_transform(x, y):
    Nux, Nuy = y[:, 0:1], y[:, 1:2]
    Nsxx, Nsyy, Nsxy = y[:, 2:3], y[:, 3:4], y[:, 4:5]
    ...
    return torch.concat([Nux, Nuy, Nsxx, Nsyy, Nsxy], axis=1)



first_layer_sizes  = [2] + [[45, 45]] * 5 + [2]
second_layer_sizes = [2] + [[45, 45, 45]] * 5 + [3]
...
plt.title("Ex1 Const E Abaqus Data NoPressure: nu vs Epochs")
plt.grid(True)
plt.tight_layout()
...
plt.title("Ex1 Const E Abaqus Data NoPressure: Training Loss vs Epochs")
plt.grid(True)
plt.tight_layout()
plt.savefig("....../..../.png", dpi=150)
plt.close()
